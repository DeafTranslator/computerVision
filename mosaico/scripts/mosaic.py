...
def readFolder():
    ...
    if imagesInMosaic%2 is not 0:
        print("The images in mosaic should be an even number")
        break
    ...
    for fld in classes:   # assuming data directory has a separate folder for each class, and that each folder is named after the class
        ...
        for fl in files:
            ...
            if len(stackImage) < imagesInMosaic:
                stackImage.append(frame)
            else:
                mosaic = mosaic(stackImage)
                stackImage = []

                if type(mosaic) is np.ndarray:
                    cv2.imshow("mosaic", mosaic)
                    tools.saveImage(name, mosaic.copy(), save_path, fld, 'mosaic')
                k = cv2.waitKey(1)
