...
from scripts.smoothing import smoothing
...
def loadVideo(video, fld, namePath):
    ...
    while( cap.isOpened() ):
        ...
        frame = frame[int(frame.shape[0]*0.24):frame.shape[0],0:int(frame.shape[0]*1)]
        ...
        if idxFrame % setup.modulu:
            ...
            blurFrame = smoothing(setup.smoothingFilters.MEDIAN, frame.copy(),setup.valueBlur[0])
            hsv = setup.cv2.cvtColor(frame.copy(), setup.cv2.COLOR_BGR2HSV)
            if setup.frameCollected > 0:
              frame = setup.myCV.drawRectangle(frame, setup.roiPts)
            # # *
            if setup.frameCollected > 0 and collect:
              l, u =  setup.myCV.getAverageColors(hsv, setup.roiPts)
              lower.append(l)
              upper.append(u)
            ...
            if not collect:
              trackingOutputWB = setup.myCV.mergeColorsImage(hsv.copy(), lower, upper)

              #   Morphological Transformations
              dilatedTracking = setup.cv2.dilate(trackingOutputWB, setup.np.ones((13, 13)))
              dilatedTracking =  setup.cv2.morphologyEx(dilatedTracking, setup.cv2.MORPH_CLOSE, setup.np.ones((13, 13)))
              ...
              grayShapes = setup.cv2.cvtColor(bitwiseResult, setup.cv2.COLOR_BGR2GRAY)
              ...
              grayShapes, wb = cfa.coverFace(grayShapes, smoothTracking)

              # Thresholding: black shapes, white background
              wbInv = setup.cv2.threshold(wb, 90,255, setup.cv2.THRESH_BINARY_INV)

              # Edge detection
              edgeDetectionImg = edd.edgeDetection(grayShapes.copy())
